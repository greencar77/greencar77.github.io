...
function FilterCore(entries, config) {
    ...
    this.populatePanel = function(panel) {
        ...
        let filteredEntries, random, mapRenderer;
        if (this.filters && this.filters.has(panel.id)) {
            ...
            if (filterSettings.mapRenderer) {
                mapRenderer = filterSettings.mapRenderer;
            }
        } else {
            filteredEntries = this.entries;
        ...
        if (mapRenderer) {
            panel.appendChild(this.createMapPane(filteredEntries, panel, mapRenderer));
        }
    ...
    this.applyFilterMap = function(elem, filterName) {
        let root = elem.parentElement.parentElement.parentElement.parentElement.parentElement;
        this.applyFilterByPanel(root, filterName);
    }
    ...
    this.applyFilterByPanel = function(panelElem, filterNameList) {
        let filterId = panelElem.id;
        console.log('applyFilterByPanel ' + filterId + ' ' + filterNameList);

        let context = this.context.get(filterId);

        //apply filter
        let filterNameArr = filterNameList.split(',');
        for (const filterName of filterNameArr) {
    ...
    this.createMapPane = function(filteredEntries, panel, mapRenderer) {
